...
# 下载图片保存路径
DIR_PATH = r'C:\Users\cc-tsr2020\Documents\teach_for_noobXi\YCMeiZiTu\MMIMG'
...
def get_urls():
    page_urls = [
        'http://www.mzitu.com/page/{cnt}'.format(cnt=cnt) for cnt in range(1, 10)]
    ...
    for page_url in page_urls:
        try:
            ...
            bs = BeautifulSoup(requests.get(
                page_url, headers=HEADERS, timeout=10).text, 'lxml').find('ul', id="pins")
            result = re.findall(r"(?<=href=)\S+", str(bs))      # 匹配所有 urls
            img_url = [url.replace('"', "") for url in result]
            print('第{0}套图有{1}张图片'.format(i, len(img_url)))
